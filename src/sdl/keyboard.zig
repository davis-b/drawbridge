...
pub const getKeycodeFromScancode = SDL_GetKeyFromScancode;
pub const getScancodeFromKeycode = SDL_GetScancodeFromKey;

pub fn getKeyFromName(name: [*c]char) !c.SDL_Keycode {
    const result = c.SDL_GetKeyFromName(name);
    if (result == c.SDLK_UNKNOWN) return error.UnrecognizedKeyName;
    return result;
}
...
pub const Keycode = extern enum(c_int) {
    UNKNOWN = 0,
    RETURN = 13,
    ESCAPE = 27,
    BACKSPACE = 8,
